...
TEST_SAMPLES = 500  # 测试样本数，完整测试集太慢，用子集快速评估
BATCH_SIZE = 8  # 根据显存调整
...
def load_test_data(num_samples: int = TEST_SAMPLES):
    """加载 WMT19 测试集"""
...
def main():
    ...
    with open("zero_shot_results.txt", "w", encoding="utf-8") as f:
        ...
        f.write("="*60 + "\n")
        for r in results:
            f.write(f"{r['model']}: BLEU={r['bleu']:.2f} ({r['status']})\n")
            if r["status"] == "success":
                f.write(f"  示例输出: {r['sample_output'][:100]}...\n")
    
    print("\n结果已保存到 zero_shot_results.txt")
